...
public class CS400Graph<T> implements GraphADT<T> {
  ...
  protected Path dijkstrasShortestPath(T start, T end) {
    if (containsVertex(start) && containsVertex(end)) {
      ...
      Path shortest = new Path(this.vertices.get(start));// pointer to the shortest path in the
                                                         // priority queue
      ArrayList<T> visitedVertices = new ArrayList<T>(); // stores a vertex to which the shortest
                                                         // path has been already discovered
      PriorityQueue<Path> pq = new PriorityQueue<Path>();// queue of various paths

      for (int i = 0; i < shortest.start.edgesLeaving.size(); i++) {
        pq.add(new Path(shortest, shortest.start.edgesLeaving.get(i)));
      }

      while (!pq.isEmpty()) {

        if (pq.peek().end.data.equals(end)) {
          return pq.peek();
        }
        visitedVertices.add(pq.peek().end.data);
        shortest = pq.poll();

        for (int i = 0; i < shortest.end.edgesLeaving.size(); i++) {
          // the step below adds a new path only if the target vertices have not been visited before
          if (!visitedVertices.contains(shortest.end.edgesLeaving.get(i).target.data)) {
            Path temp = new Path(shortest, shortest.end.edgesLeaving.get(i));
            pq.add(temp);
          }
        }
      }
