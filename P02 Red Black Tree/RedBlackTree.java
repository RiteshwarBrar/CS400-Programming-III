...
public class RedBlackTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
  ...
  public boolean insert(T data) throws NullPointerException, IllegalArgumentException {
    ...
    if (root == null) {
      ...
      root.isBlack = true; // the root node is always set to be black after each new insertion
      return returnValue;
    }
  }
  ...
  private boolean insertHelper(Node<T> newNode, Node<T> subtree) {
    ...
    // store newNode within left subtree of subtree
      ...
      if (subtree.leftChild == null) { // left subtree empty, add here
        ...
        enforceRBTreePropertiesAfterInsert(newNode); // calling this method to check and resolve any
                                                     // RBT violations
        return true;
    ...
    // store newNode within the right subtree of subtree
      ...
      if (subtree.rightChild == null) { // right subtree empty, add here
        ...
        newNode.parent = subtree;
        enforceRBTreePropertiesAfterInsert(newNode); // to check and resolve any RBT violations
        return true;
        // otherwise continue recursive search for location to insert
      } else
  ...
  private void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException {
    ...
    if (child.equals(parent.leftChild)) {
      ...
      if (parent.parent != null) { // checks whether the parent node is a root node
        if (parent.isLeftChild()) {
          parent.parent.leftChild = child;
          // replaces parent with child
        } else {
      ...
      } else {// if parent is the root node then root is changed to child
        root = child;
      }
      // stores right child of the child temporarily
      child.rightChild = parent;
      parent.leftChild = temp;// the rightChild of the child becomes the leftChild of the parent
      parent.parent = child;
      ...
      parent.parent = child;
    } else {// exception is thrown when the parameters aren't related
      throw new IllegalArgumentException();
    }
  }

  /**
   * This method checks the validity of a Red-Black Tree after a new node is added to the tree. it
   * follows the rules of a typical Red-Black Tree and this method recolors as well as rotates the
   * nodes of the tree when required to do so.
   * 
   * @param node newly added node
   */
  private void enforceRBTreePropertiesAfterInsert(Node<T> node) {

    if (node.equals(root) || node.parent.parent == null) {

      return;
    }
    if (!node.parent.isBlack) {
      if (node.parent.isLeftChild()
          && (node.parent.parent.rightChild == null || node.parent.parent.rightChild.isBlack)) {
        // when the uncle node of the newly added node is black
        if (!node.isLeftChild()) {// checks for triangle
                                  // scenario

          rotate(node, node.parent);
          enforceRBTreePropertiesAfterInsert(node.leftChild); // recursive call helps resolve the
                                                              // line
          // scenario after rotation is complete in
          // the triangle scenario
        } else {// block to solve the line scenario
          ...
          && (node.parent.parent.leftChild == null || node.parent.parent.leftChild.isBlack)) {
        // when the uncle node of the newly added node is black
      ...
      else if (node.parent.isLeftChild() && !node.parent.parent.rightChild.isBlack) {
        // when the uncle node of the newly added node is red
        node.parent.parent.isBlack = false;
        node.parent.isBlack = true;
        node.parent.parent.rightChild.isBlack = true;
      }

      else if (!node.parent.isLeftChild() && !node.parent.parent.leftChild.isBlack) {// when the
        // uncle node of the newly added node is red
        node.parent.parent.isBlack = false;
        node.parent.isBlack = true;
